...
int main() {
    ...
    for (beta_deg = -20.0; beta_deg <= 20.0; beta_deg += 0.5) {

        cout << "beta = " << beta_deg << endl;
...
//        ostringstream rem_str;
//        rem_str << (double)(R_em);
        ostringstream beta_str;
        beta_str << (double)(beta_deg);

        string name = "int_beta=" + beta_str.str();
        string path = "dats/interpulse/";
        ...
        for (double phi_t = 20.0; phi_t >= -20.0; phi_t -= 0.5) { // Phase switch
//            cout << "PHI: " << phi_t << endl;
            PHI0 = phi_t * PI / 180.0;
            findInitPoints (PHI0);
